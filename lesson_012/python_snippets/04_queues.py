        ...
        for worm in range(self.worms):
            ...
            if fish is None:
                ...
                if self.catcher.full():
                    print(colored(f'{self.name}, {worm}: приемщик полон !!!', color='red'), flush=True)
                # Этот метод у очереди - атомарный и блокирующий
                # Поток приостанавливается, пока нет места в очереди
                self.catcher.put(fish)
                print(colored(f'{self.name}, {worm}: наконец-то отдал {fish} приемщику', color='green'), flush=True)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(colored(f'Приемщик принял {fish} и положил в садок', color='yellow'), flush=True)
                self.fish_tank[fish] += 1
            except queue.Empty:
                print(colored(f'Приемщику нет рыбы в течении 1 секунды', color='yellow'), flush=True)
